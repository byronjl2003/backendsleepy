<?php
  ...
  private function getdatah($email,$email1,$email2) {
     ...
     */
     ...
     $query = $this->_conn->prepare("select T.temperatura,T.humedad,T.movimiento,T.luz,T.sonido,T.ronquido FROM(select D.fecha,D.temperatura,D.humedad,D.movimiento,D.luz,D.sonido,D.ronquido FROM master_det M INNER JOIN data D ON M.id = D.id WHERE M.correo = ?) T WHERE T.fecha BETWEEN ? AND ? ORDER BY T.fecha DESC LIMIT 1;");
