<?php
  ...
  private function getdatah($email,$fecha1,$fecha2) {
     ...
     */
     ...
     $query = $this->_conn->prepare("selectfech T.temperatura,T.humedad,T.movimiento,T.luz,T.sonido,T.ronquido FROM(select D.fecha,D.temperatura,D.humedad,D.movimiento,D.luz,D.sonido,D.ronquido FROM master_det M INNER JOIN data D ON M.id = D.id WHERE M.correo = ?) T WHERE T.fecha BETWEEN ? AND ? ORDER BY T.fecha ASC;");
